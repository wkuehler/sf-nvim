...
describe("runner.scratch", function()
	it("creates a read-only scratch buffer in a split and reuses it by name", function()
		local b1 = runner.scratch({ name = "sf://spec", lines = { "one", "two" }, filetype = "soqlresult" })
		assert.equals(b1, vim.api.nvim_get_current_buf())
		assert.equals("nofile", vim.bo[b1].buftype)
		assert.is_false(vim.bo[b1].modifiable)
		assert.same({ "one", "two" }, vim.api.nvim_buf_get_lines(b1, 0, -1, false))
		local wins = #vim.api.nvim_list_wins()

		local b2 = runner.scratch({ name = "sf://spec", lines = { "three" } })
		assert.equals(b1, b2)
		assert.equals(wins, #vim.api.nvim_list_wins())
		assert.same({ "three" }, vim.api.nvim_buf_get_lines(b1, 0, -1, false))
		assert.truthy(vim.fn.maparg("q", "n", false, true).buffer == 1)
		vim.cmd("close")
	end)
end)
